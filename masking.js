...
var simpleMasksPt1Stage = {
    ...
    // 4. FULL DATE REPLACEMENT BY TAKING AN ARBITRARY DATETIME OF 01-Jan-2021 AND ADDING A RANDOM AMOUNT UP TO ONE YEAR MAX
    'settlement_date': {'$add': [
...
var redactFieldsStage = {
    // 1. EXCLUDE SUB-DOCUMENT DATA BASED ON A FIELD'S VALUE, eg. if customer_info.category = SENSITIVE
    '$cond': {
...
replace_pipeline.push(
    {'$merge': {'into': { 'db': 'ms_masking', 'coll': 'payments'}, 'on': '_id',  'whenMatched': 'replace', 'whenNotMatched': 'fail'}}
);
...
db.createRole(
   {
     role: "findRedactedView", 
     privileges: [
        { resource: { db: "ms_masking", collection: "payments_redacted_view" },  actions: [ "find" ] }
     ],
     roles: []
   }
)
...
db.createUser(
  {
    ...
    roles: [ { role: "findRedactedView", db: "ms_masking" } ]
  }
)
